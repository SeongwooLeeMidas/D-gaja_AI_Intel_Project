...
# 모델과 가중치 파일 경로
model_xml = "/home/ubuntu/workspace1/otx_detection/outputs/deploy/model/model.xml"
model_bin = "/home/ubuntu/workspace1/otx_detection/outputs/deploy/model/model.bin"
...
while cap.isOpened():
    ...
    image_resized = cv2.resize(frame, (384, 384))  # 입력 크기를 모델에 맞춤
    ...
    for detection in result[0]:
        ...
        if confidence > 0.5:  # 신뢰도가 0.5보다 큰 경우만 고려
            x_min = int(x_min * original_width / 384)
            y_min = int(y_min * original_height / 384)
            x_max = int(x_max * original_width / 384)
            y_max = int(y_max * original_height / 384)
