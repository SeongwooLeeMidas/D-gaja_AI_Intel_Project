...
HOST = '192.168.5.82'        # 서버 IP 주소 (빈 문자열은 현재 호스트의 모든 네트워크 인터페이스를 의미)
PORT = 8485      # 사용할 포트 번호
...
def info_ui (frame):
    global key_ui
    cv2.putText(frame, "D-gaja", (80, 70), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 255), 2)
    cv2.putText(frame, "Press Key!", (120, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
    cv2.putText(frame, "A : auto-mode, M: modify-degree, Q: quit-program", (1, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (0, 255, 255), 1)
    key = cv2.waitKey(1)
    if key == ord('q'):
        key_ui = 'q'
    elif key == ord('a'):
